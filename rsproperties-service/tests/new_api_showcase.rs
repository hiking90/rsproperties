...
async fn test_new_api_showcase() -> anyhow::Result<()> {
    ...
    // Set string property using set_str()
    rsproperties::set_str("app.config.name", "MyApplication")?;
    ...
    let app_name = rsproperties::get("app.config.name");
    assert_eq!(app_name, "MyApplication");

    // Parse properties to specific types using get_parsed()
    let version_code: i32 = rsproperties::get_parsed("app.config.version_code")?;
    assert_eq!(version_code, 123);

    let version_minor: u8 = rsproperties::get_parsed("app.config.version_minor")?;
    assert_eq!(version_minor, 4);

    let memory_limit: f64 = rsproperties::get_parsed("app.config.memory_limit")?;
    assert!((memory_limit - 512.5).abs() < f64::EPSILON);

    let debug_enabled: bool = rsproperties::get_parsed("app.config.debug_enabled")?;
    assert!(debug_enabled);

    // Parse string that was set as string
    let api_timeout: u32 = rsproperties::get_parsed("app.config.api_timeout")?;
    ...
    let existing_timeout: u32 =
        rsproperties::get_parsed_with_default("app.config.api_timeout", 5000);
    assert_eq!(existing_timeout, 30000); // Should return actual value, not default

    // Get with defaults for non-existent properties
    let missing_prop: i32 = rsproperties::get_parsed_with_default("app.config.nonexistent", 42);
    assert_eq!(missing_prop, 42); // Should return default

    // Get with defaults for properties with invalid format
    rsproperties::set_str("app.config.invalid_number", "not_a_number")?;
    let invalid_parsed: i32 =
        rsproperties::get_parsed_with_default("app.config.invalid_number", 999);
    ...
    let missing_result: Result<i32, _> = rsproperties::get_parsed("app.config.does_not_exist");
    assert!(missing_result.is_err());

    // Attempting to parse invalid format should fail
    let invalid_result: Result<i32, _> = rsproperties::get_parsed("app.config.invalid_number");
...
async fn test_android_style_usage() -> anyhow::Result<()> {
    ...
    rsproperties::set_str("ro.build.version.sdk", "34")?;
    rsproperties::set_str("ro.debuggable", "0")?;
    rsproperties::set_str("dalvik.vm.heapsize", "512m")?;
    rsproperties::set_str("debug.my_app.log_level", "verbose")?;

    // Application code reading these properties

    // SDK version as integer
    let sdk_version: i32 = rsproperties::get_parsed_with_default("ro.build.version.sdk", 28);
    assert_eq!(sdk_version, 34);

    // Debug flag as boolean (0/1 pattern)
    let is_debuggable: i32 = rsproperties::get_parsed_with_default("ro.debuggable", 0);
    ...
    rsproperties::set_str("dalvik.vm.heapsize.mb", "512")?;
    let heap_size: u32 = rsproperties::get_parsed_with_default("dalvik.vm.heapsize.mb", 256);
    assert_eq!(heap_size, 512);

    // String property with fallback
    let log_level = rsproperties::get_with_default("debug.my_app.log_level", "info");
    assert_eq!(log_level, "verbose");

    // Non-existent property with fallback
    let unknown_feature: i32 = rsproperties::get_parsed_with_default("vendor.unknown.feature", 0);
...
async fn test_type_conversion_showcase() -> anyhow::Result<()> {
    ...
    rsproperties::set_str("config.numeric_value", "42")?;

    // Parse as different integer types
    let as_i8: i8 = rsproperties::get_parsed("config.numeric_value")?;
    let as_i16: i16 = rsproperties::get_parsed("config.numeric_value")?;
    let as_i32: i32 = rsproperties::get_parsed("config.numeric_value")?;
    let as_i64: i64 = rsproperties::get_parsed("config.numeric_value")?;
    let as_u32: u32 = rsproperties::get_parsed("config.numeric_value")?;
    let as_u64: u64 = rsproperties::get_parsed("config.numeric_value")?;
    ...
    let as_f32: f32 = rsproperties::get_parsed("config.numeric_value")?;
    let as_f64: f64 = rsproperties::get_parsed("config.numeric_value")?;

    assert_eq!(as_f32, 42.0);
    assert_eq!(as_f64, 42.0);

    // Parse as string
    let as_string: String = rsproperties::get_parsed("config.numeric_value")?;
    assert_eq!(as_string, "42");

    // Set floating point and parse back
    rsproperties::set("config.pi", &std::f64::consts::PI)?;
    let pi_back: f64 = rsproperties::get_parsed("config.pi")?;
    assert!((pi_back - std::f64::consts::PI).abs() < 1e-10);

    // Set boolean values
    rsproperties::set("config.feature_flag", &true)?;
    let flag_value = rsproperties::get("config.feature_flag");
    assert_eq!(flag_value, "true");

    // Note: Parsing "true"/"false" strings as bool works with Rust's FromStr
    rsproperties::set_str("config.bool_string", "false")?;
    let bool_parsed: bool = rsproperties::get_parsed("config.bool_string")?;
    assert!(!bool_parsed);
...
async fn test_safe_programming_patterns() -> anyhow::Result<()> {
    ...
    let safe_timeout = rsproperties::get_parsed_with_default("config.timeout", 5000u32);
    assert_eq!(safe_timeout, 5000); // Default because property doesn't exist

    // Pattern 2: Explicit error handling
    match rsproperties::get_parsed::<i32>("config.nonexistent") {
    ...
    rsproperties::set_str("config.validation_test", "123")?;
    let validated: Result<u32, _> = rsproperties::get_parsed("config.validation_test");
    match validated {
        Ok(val) => {
            assert_eq!(val, 123);
    ...
    let config_value = rsproperties::get_parsed_with_default::<i32>(
        "config.primary",
        rsproperties::get_parsed_with_default("config.secondary", 42),
    );
    ...
    rsproperties::set_str("app.name", "TestApp")?;
    ...
    let config = AppConfig {
        name: rsproperties::get_with_default("app.name", "DefaultApp"),
        version: rsproperties::get_parsed_with_default("app.version", 1),
        debug: rsproperties::get_parsed_with_default("app.debug", false),
        timeout: rsproperties::get_parsed_with_default("app.timeout", 5000),
    };
            ...
            tokio::spawn(async move {
                ...
                match i % 3 {
                    0 => {
                        ...
                        let value: i32 = rsproperties::get_parsed(&prop_name)?;
                        assert_eq!(value, i * 100);
                    }
                    1 => {
                        rsproperties::set(&prop_name, &(i as f64 * 1.5))?;
                        let value: f64 = rsproperties::get_parsed(&prop_name)?;
...
async fn test_complete_feature_summary() -> anyhow::Result<()> {
    ...
    rsproperties::set_str("demo.string_prop", "Hello World")?;
    rsproperties::set("demo.int_prop", &42i32)?;
    rsproperties::set("demo.float_prop", &2.5f64)?;
    rsproperties::set("demo.bool_prop", &true)?;
    println!("   ✅ Properties set using set_str() and set() functions");

    // Feature 2: Type-Safe Property Parsing
    println!("📊 Feature 2: Type-Safe Property Parsing");
    let string_val: String = rsproperties::get_parsed("demo.string_prop")?;
    let int_val: i32 = rsproperties::get_parsed("demo.int_prop")?;
    let float_val: f64 = rsproperties::get_parsed("demo.float_prop")?;
    let bool_val: bool = rsproperties::get_parsed("demo.bool_prop")?;
    ...
    let safe_int: i32 = rsproperties::get_parsed_with_default("demo.missing_prop", 999);
    let safe_str: String =
        rsproperties::get_parsed_with_default("demo.missing_string", "default".to_string());
    ...
    let error_result: Result<i32, _> = rsproperties::get_parsed("demo.nonexistent");
    assert!(error_result.is_err());

    rsproperties::set_str("demo.invalid_int", "not_a_number")?;
    let parse_error: Result<i32, _> = rsproperties::get_parsed("demo.invalid_int");
    ...
    let traditional_get = rsproperties::get("demo.string_prop");
    let traditional_with_default = rsproperties::get_with_default("demo.int_prop", "0");
