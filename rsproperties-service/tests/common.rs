...
static SERVICES: OnceCell<(ActorRef<SocketService>, ActorRef<PropertiesService>)> = OnceCell::const_new();

async fn inti() -> (ServiceContext<SocketService>, ServiceContext<PropertiesService>) {
...
pub async fn init_test() -> (ActorRef<SocketService>, ActorRef<PropertiesService>) {
    SERVICES.get_or_init(|| async {
        let (sender, receiver) = tokio::sync::oneshot::channel::<(ActorRef<SocketService>, ActorRef<PropertiesService>)>();
        ...
        std::thread::spawn(move || {
            ...
            runtime.block_on(async {
                ...
                let _ = sender.send((services.0.actor_ref.clone(), services.1.actor_ref.clone()));

                let (res1, res2) = tokio::join!(
                    services.0.join_handle,
                    services.1.join_handle,
                );
        ...
        receiver.await
            .expect("Failed to receive services")
    }).await
    .clone()
}
