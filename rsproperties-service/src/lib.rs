...
pub async fn run(
...
) -> Result<(ServiceContext<SocketService>, ServiceContext<PropertiesService>), Box<dyn std::error::Error>> {
    rsproperties::init(config);

    let properties_service = properties_service::run(
        property_contexts_files, build_prop_files,
    );

    // Initialize the socket service
    let socket_service = socket_service::run(
        SocketServiceArgs {
    ...
    let _ = socket_service.actor_ref.ask(ReadyMessage{})
        .await
        .map_err(|e| format!("Failed to start socket service: {}", e))?;
    let _ = properties_service.actor_ref.ask(ReadyMessage{})
        .await
